"""
Generate docs/samples/acme-sample.pptx — a 5-slide demo deck for UC3.

Theme: Acme's "SageGlass" dynamic electrochromic glazing — a realistic
...
Output: docs/samples/acme-sample.pptx
"""
...
OUT = Path(__file__).resolve().parent.parent / "docs" / "samples" / "acme-sample.pptx"
...
SLIDES = [
    {
        ...
        "subtitle": "Acme Insights · Innovation Podcast",
        "bullets": [
            "Electrochromic glass that tints on demand",
            "Reduces HVAC energy use by up to 20%",
            "A Acme product since 2012",
        ],
        "notes": (
            "This is the opening slide. Introduce SageGlass as Acme's flagship "
    ...
    },
    {
        ...
        "notes": (
            ...
            "'future' if they partner with Acme."
        ),
    },
]
...
def _add_footer(slide, n: int, total: int) -> None:
    ...
    tf = box.text_frame
    p = tf.paragraphs[0]
    p.text = f"Acme Insights   ·   Slide {n} of {total}"
    p.font.size = Pt(10)
